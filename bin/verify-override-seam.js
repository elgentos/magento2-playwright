...
function main() {
	...
	try {
		...
		const checks = [
			...
			// member was reached.
			[
				`SEAM_PROTECTED=role:heading:${loginTitle}`,
				'protected member was not reachable, or super getter was lost',
			],
			[
				`SEAM_COMPOSE=${COMPOSE_SENTINEL}`,
				'cross-POM composition did not pick up the store override of the composed POM',
			],
		];
		...
		console.log(
			'PASS: store override shadows the base POM, can extend it, and is picked up when composed by another base POM',
		);
	} finally {
