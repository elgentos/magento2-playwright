...
async function createCoupons() {
	...
	if (!baseUrl || !adminUsername || !adminPassword) {
		console.error(
			'Missing required .env variables: PLAYWRIGHT_BASE_URL, MAGENTO_ADMIN_USERNAME, MAGENTO_ADMIN_PASSWORD',
		);
	...
	const api = (method, url, payload) =>
		apiRequest(method, url, token, baseUrl, httpCredentials, payload);
	...
	const websiteIds = websiteInfo.filter((w) => w.name !== 'admin').map((w) => w.id);
