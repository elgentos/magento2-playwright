...
/**
 * - Add First Address to Account
 * - Add another address
 * - edit existing address
 * - delete an address
 * - Magento 2 newsletter subscriptions
 */
...
test.describe('Address actions', () => {
   ...
   */
  customTest('Add_the_first_address', {tag: '@address'}, async({userPage}, testInfo) => {
   ...
   */
  customTest('Add_another_address', {tag: ['@address']}, async ({userPage}) => {
    await userPage.page.goto(slugs.account.addressNewSlug);
    const accountPage = new AccountPage(userPage.page);
   ...
   */
  customTest('Update_existing_address', {tag: ['@address']}, async ({userPage}) => {
    const accountPage = new AccountPage(userPage.page);
   ...
   */
  customTest('Delete_existing_address', {tag: ['@address']}, async ({userPage}) => {
    const accountPage = new AccountPage(userPage.page);
    await userPage.page.goto(slugs.account.addressBookSlug);
 ...
 */
test('Password_change_for_existing_account',{ tag: '@password',}, async ({page, browserName}, testInfo) => {
 ...
 */
customTest('Update_newsletter_subscription', {tag: ['@newsletter']}, async ({userPage, browserName}) => {
