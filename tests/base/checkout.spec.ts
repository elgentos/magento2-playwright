 ...
 */
customTest('Address_is_filled_in_at_checkout_for_user', {tag: ['@checkout', '@user']}, async ({userProductPage}) => {
  await userProductPage.page.goto(slugs.checkout.checkoutSlug);
 ...
 */
customTest('User_place_order_for_simple_product', {tag: ['@checkout', '@user']}, async ({userProductPage, page}, testInfo) => {
 ...
 */
customTest('Remove_coupon_code_in_checkout', {tag: ['@checkout', '@coupon-code']}, async ({productPage, browserName}) => {
  await productPage.page.goto(slugs.checkout.checkoutSlug);
  const checkout = new CheckoutPage(productPage.page);
  const browserEngine = browserName?.toUpperCase() || "UNKNOWN";
  let discountCode = process.env[`MAGENTO_COUPON_CODE_${browserEngine}`];
 ...
 */
customTest('Invalid_coupon_code_in_checkout_is_rejected', {tag: ['@checkout', '@coupon-code']}, async ({productPage, browserName}) => {
    await productPage.page.goto(slugs.checkout.checkoutSlug);  
    const checkout = new CheckoutPage(productPage.page);
...
/**
 * @feature Price calculations in checkout
 * @scenario The calculated price in the checkout should be correct
 * @given I have a product in my cart
 * @when I navigate to the checkout page
 *  @and I select a shipping option
 *  @and wait for totals to update
 * @when I add all the elements of the total price, it should be equal to what is displayed
 */
 ...
 */
customTest('Guest can select different payment methods', { tag: ['@checkout', '@payment-methods'] }, async ({ productPage }) => {
  const checkoutPage = new CheckoutPage(productPage.page);
