...
class MagewireUtils {
   ...
   */
  private static readonly traffic = new WeakMap<Page, MagewireTraffic>();
  ...
  private get traffic(): MagewireTraffic {
  ...
  async waitForMagewireRequests(): Promise<void> {
    ...
    while (Date.now() - start <= maxWaitTime) {
      const quiet = traffic.inFlight.size === 0 && Date.now() - traffic.lastActivityAt >= quietPeriod;
