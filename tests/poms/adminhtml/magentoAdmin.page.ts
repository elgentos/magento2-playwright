...
class MagentoAdminPage {
  ...
  async enableMultipleAdminLogins() {
    ...
    await this.page.locator(UIReference.configurationPage.allowMultipleLoginsSelectField).selectOption({ label: inputValues.adminLogins.allowMultipleLogins });
  ...
  async disableLoginCaptcha() {
    ...
    await this.page.locator(UIReference.configurationPage.captchaSettingSelectField).selectOption({ label: inputValues.captcha.captchaDisabled });
    await this.page.getByRole('button', { name: UIReference.configurationPage.saveConfigButtonLabel }).click();
    await this.page.waitForLoadState('networkidle');
  }
}
