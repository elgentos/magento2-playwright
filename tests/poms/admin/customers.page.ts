...
class AdminCustomers {
	...
	async checkIfCustomerExists(email: string) {
		const mainMenuCustomersButton = this.page
			.getByRole('link', { name: UIReference.text.admin.common.customers })
			.first();
		const allCustomersLink = this.page.getByRole('link', {
			name: UIReference.text.admin.common.navigation.allCustomers,
		});
		const customersSearchField = this.page.getByRole('textbox', {
			name: UIReference.text.admin.common.tableSearch,
		});

		// loop clicking the 'Customers' button until clicking it show the subnavigation
		await expect(async () => {
			await mainMenuCustomersButton.press('Enter');
			await expect(allCustomersLink, `Link to "All Customers" is visible`).toBeVisible({
				timeout: 5000,
			});
		}).toPass();
		...
		await this.page.waitForURL(
			slugToRegex(`/${requireEnv('MAGENTO_ADMIN_SLUG')}${slugs.admin.customers.index}`),
		);
		...
		const emailIsFound = await this.page
		...
		await this.page
			.getByRole('button', { name: UIReference.text.shared.buttons.clearAll })
			.click();
	 ...
	 */
	async createNewCustomerAccount(firstName: string, lastName: string, email: string) {
		const createNewCustomersLink = this.page.getByRole('button', {
			name: UIReference.text.admin.customers.createNew,
		});
		await createNewCustomersLink.click();

		// Wait for URL. If loading symbol is visible, wait for it to go away
		await this.page.waitForURL(
			slugToRegex(`/${requireEnv('MAGENTO_ADMIN_SLUG')}${slugs.admin.customers.new}`),
		);
		if (await this.page.locator(UIReference.selectors.shared.spinner).isVisible()) {
			await this.page
				.locator(UIReference.selectors.shared.spinner)
		...
		const accountCreationFirstNameField = this.page.getByLabel(
			UIReference.text.shared.forms.firstName,
		);
		const accountCreationLastNameField = this.page.getByLabel(
			UIReference.text.shared.forms.lastName,
		);
		const accountCreationEmailField = this.page.getByLabel(
			UIReference.text.shared.forms.email,
			{ exact: true },
		);
		const accountCreationConfirmButton = this.page.getByRole('button', {
			name: UIReference.text.admin.customers.saveAndContinue,
		});
		const customersSearchField = this.page.getByRole('textbox', {
			name: UIReference.text.admin.common.tableSearch,
		});
		...
		// Optional fields:
		const allowBulkPurchaseSwitcher = this.page
	...
	async approveAccount(email: string) {
		...
		await expect(async () => {
			await this.page
				.getByText(outcomeMarker.customerOverviewPage.searchResultsFoundText)
				.first();
		}).toPass();
