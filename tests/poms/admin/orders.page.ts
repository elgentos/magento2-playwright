...
class AdminOrders {
	...
	async checkIfOrderExists(orderNumber: string) {
		const mainMenuSalesButton = this.page.getByRole('link', {
			name: UIReference.text.admin.common.navigation.sales,
		});
		const ordersButtonLink = this.page
			.getByRole('link', { name: UIReference.text.admin.common.navigation.orders })
			.first();
		...
		const ordersSearchField = this.page.getByRole('textbox', {
			name: UIReference.text.admin.common.tableSearch,
		});

		// Wait for URL. If loading symbol is visible, wait for it to go away
		await this.page.waitForURL(
			slugToRegex(`/${requireEnv('MAGENTO_ADMIN_SLUG')}${slugs.admin.orders.index}`),
		);
		...
		await expect(
			this.page.getByRole('cell', { name: orderNumber }).locator('div'),
		).toBeVisible();
	}
}
