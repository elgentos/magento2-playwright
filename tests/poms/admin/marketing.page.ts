...
class AdminMarketing {
	...
	async addCartPriceRule(magentoCouponCode: string) {
		...
		const mainMenuMarketingButton = this.page.getByRole('link', {
			name: UIReference.text.admin.common.navigation.marketing,
		});
		const cartPriceRulesLink = this.page.getByRole('link', {
			name: UIReference.text.admin.common.navigation.cartPriceRules,
		});
		...
		await expect(async () => {
			...
			await expect(
				cartPriceRulesLink,
				`Button for Cart Price Rules is visible`,
			).toBeVisible();
		}).toPass();

		await cartPriceRulesLink.click();

		const addCartPriceRuleButton = this.page.getByRole('button', {
			name: UIReference.text.admin.cartPriceRules.addRule,
		});
		await addCartPriceRuleButton.waitFor();

		// Use search field to check for coupon codes.
		const couponSearchField = this.page.locator(
			UIReference.selectors.admin.common.couponSearch,
		);
		await couponSearchField.fill(magentoCouponCode);
		await this.page
			.getByRole('button', { name: UIReference.text.shared.buttons.search })
			.click();

		await expect(
			this.page.getByText(UIReference.text.admin.common.recordsFound),
			`Search results text visible`,
		).toBeVisible();

		const couponCellField = this.page.getByRole('cell', {
			name: outcomeMarker.magentoAdmin.noResultsFoundText,
		});

		if (await couponCellField.isHidden()) {
			const couponStatusField = this.page
				.locator('tr')
				.filter({ hasText: magentoCouponCode })
				.first();
			...
			if (couponStatus.includes(UIReference.text.admin.cartPriceRules.couponActive)) {
				...
				const activeStatusSwitcher = this.page
					.locator(UIReference.selectors.admin.cartPriceRules.activeStatusSwitcher)
					.first();
				const activeStatusLabel = this.page
					.locator(UIReference.selectors.admin.cartPriceRules.activeStatusLabel)
					.first();

				await expect(activeStatusLabel, `Active/Disable toggle is visible`).toBeVisible();
				await activeStatusSwitcher.click();

				const saveCouponButton = this.page.getByRole('button', {
				...
				await expect(
					this.page
						...
						.filter({ hasText: outcomeMarker.magentoAdmin.couponRuleSavedText }),
					"Message 'you saved the rule' is visible",
				).toBeVisible();
			...
			const customerGroupsSelector = this.page.getByLabel(
				UIReference.text.admin.cartPriceRules.customerGroups,
				{ exact: true },
			);
			...
			await this.page
				.getByRole('textbox', { name: UIReference.text.admin.cartPriceRules.ruleName })
				.fill(magentoCouponCode);
			await this.page
				.locator(UIReference.selectors.admin.cartPriceRules.couponType)
				.selectOption({ label: inputValues.coupon.couponType });
			await this.page
				.getByLabel(UIReference.text.admin.cartPriceRules.couponCode)
				.fill(magentoCouponCode);

			await this.page
				.getByText(UIReference.text.admin.cartPriceRules.actionsSubtitle, { exact: true })
				.click();
			await this.page
		...
		const clearSearchButton = this.page.getByRole('button', {
			name: UIReference.text.admin.cartPriceRules.clearSearch,
		});
