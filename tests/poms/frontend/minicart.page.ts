...
export class MiniCartPage {
	...
	async removeProductFromMinicart(product: string) {
		...
		let removeProductMiniCartButton = this.page.getByLabel(
			`${UIReference.text.frontend.minicart.removeProduct} "${UIReference.text.frontend.product.simpleProduct}"`,
		);
		// ensure button is visible
		await removeProductMiniCartButton.waitFor();
		await removeProductMiniCartButton.click();
		await expect(
			removeProductMiniCartButton,
			`Button to move product from minicart is no longer visible`,
		).toBeHidden();
		await expect(
			this.page.getByText(UIReference.text.frontend.minicart.empty),
			`Minicart shows text "Cart is empty"`,
		).toBeVisible();
	}
	...
	async updateProduct(amount: string) {
		let productQuantityChangedNotification =
			outcomeMarker.miniCart.productQuantityChangedConfirmation;
		...
		let productQuantityInCart = await this.page
	...
	async checkPriceWithProductPage() {
		const priceOnPage = await this.page
		...
		const priceInMinicart = await productListing
			.locator(UIReference.selectors.frontend.minicart.price)
			.first()
			.textContent();
		expect(
			priceOnPage,
			`Expect these prices to be the same: priceOnpage: ${priceOnPage} and priceInMinicart: ${priceInMinicart}`,
		).toBe(priceInMinicart);
	}
}
