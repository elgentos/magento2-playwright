 ...
 */
export function getPlaywrightRequestConfig(
	rawUrl = requireEnv('PLAYWRIGHT_BASE_URL'),
): PlaywrightRequestConfig {
...
export function getPlaywrightApiRequestConfig(
	rawUrl = getEnvOrFallback('PLAYWRIGHT_API_BASE_URL', 'PLAYWRIGHT_BASE_URL'),
): PlaywrightRequestConfig {
	const parsedUrl = new URL(rawUrl);
	const username = decodeURIComponent(parsedUrl.username);
	const password = decodeURIComponent(parsedUrl.password);
	...
	return {
		baseURL: parsedUrl.toString(),
		...(username || password
			? { httpCredentials: { username, password, send: 'always' as const } }
			: {}),
	};
}
